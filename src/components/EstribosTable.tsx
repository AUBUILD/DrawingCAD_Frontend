...
import type { DevelopmentIn, SpanIn, StirrupsDistributionIn } from '../types';
import { type StirrupsABCR, formatStirrupsABCR, parseStirrupsABCR, pickDefaultABCRForH, normalizeDiaKey } from '../utils/stirrupsUtils';

interface EstribosTableProps {
  dev: DevelopmentIn;
  updateSpanStirrups: (spanIdx: number, patch: Partial<StirrupsDistributionIn>) => void;
  updateSpanStirrupsSection: (spanIdx: number, patch: Record<string, unknown>) => void;
  stirrupsAbcrEdits: Record<string, string>;
  setStirrupsAbcrEdits: React.Dispatch<React.SetStateAction<Record<string, string>>>;
  setABCRField: (spanIdx: number, st: StirrupsDistributionIn, ct: string, side: 'L' | 'R', field: string, raw: string) => void;
}
...
const EstribosTable: React.FC<EstribosTableProps> = ({
  ...
  const getSt = (s: SpanIn): Partial<StirrupsDistributionIn> => s.stirrups ?? {};
  ...
  const getABCR = (st: Partial<StirrupsDistributionIn>, key: 'left_spec' | 'center_spec' | 'right_spec'): StirrupsABCR => {
            ...
            return (
              <div className="cell" key={`st-case-${i}`}>
                <select
                  className="cellInput"
                  value={caseTypeOf(st)}
                  onChange={(e) => updateSpanStirrups(i, { case_type: e.target.value as StirrupsDistributionIn['case_type'] })}
                >
              ...
              <div className="cell" key={`st-mode-${i}`}>
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (ct === 'asim_uno') {
                      updateSpanStirrups(i, { design_mode: m, left_spec: spec, center_spec: spec, right_spec: undefined });
                    } else {
                      updateSpanStirrups(i, { design_mode: m, left_spec: spec, right_spec: spec, center_spec: undefined });
                    }
                  }}
                >
            ...
            return (
              <div className="cell" key={`st-single-${i}`}>
                <select
                  ...
                  onChange={(e) => updateSpanStirrups(i, { single_end: (e.target.value || undefined) as 'left' | 'right' | undefined })}
                >
                  <option value="">—</option>
          ...
          { [
            ...
            <React.Fragment key={`st-abcr-row-${row.f}`}>
              ...
              {spans.map((s: SpanIn, si: number) => {
                ...
                const kL = mkAbcrKey(si, 'L', row.f as 'A' | 'b' | 'B' | 'c' | 'C' | 'R');
                const kR = mkAbcrKey(si, 'R', row.f as 'A' | 'b' | 'B' | 'c' | 'C' | 'R');
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                        ...
                        <input
                          ...
                          onBlur={(e) => setABCRField(si, st as StirrupsDistributionIn, ct, 'L', row.f, e.target.value)}
                        />
                      </div>
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                        ...
                        <input
                          ...
                          onBlur={(e) => setABCRField(si, st as StirrupsDistributionIn, ct, 'R', row.f, e.target.value)}
                        />
                      </div>
                    </div>
                  </div>
...
export default React.memo(EstribosTable);
